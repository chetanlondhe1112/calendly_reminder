...
last_date=todays_date-dt.timedelta(90)
max_date=todays_date+dt.timedelta(days=30)
"""
for i in range(9,12):
    if i == 9:
        is1 = "0" + "9"
    ...
    params = {'count':'100','min_start_time': '2023-09-01T12:30:00.000000Z','max_start_time': max_date}
    ...
    """
params = {'count':'100','min_start_time': todays_date,'max_start_time': max_date}
...
while i < len(data1):
  ...
  i = i+1
#print(data2)
data2=data2.sort_values(by="meetingtime",ascending=True)
data2.to_csv('client_meeting.csv',index=False, encoding='utf-8')
data2['meetingtime']=pd.to_datetime(data2['meetingtime'])
dbm.upload_meeting_data(df=data2,table_name=meeting_data_tbl,if_exists="append")
print("Meeting data updated")
data2 = pd.read_csv('client_meeting.csv',keep_default_na=False )

#read master client data
data3 = pd.read_csv('F:/Project-11/calendly_reminder/scratch/trails/cmmv1/files/cdeclients.csv',keep_default_na=False )
data4 = pd.DataFrame()
j = 1
...
#dbm.upload_to_table(df=data4,table_name=clients_notmet_data_tbl,if_exists="append")
print("Clients notmet data updated")
